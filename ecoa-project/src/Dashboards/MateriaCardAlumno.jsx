...
function Materia({materia}){
    ...
    const handleViewMore = () =>{
        ...
        navigate("/ecoa/"+materia.group_number);
    }

    return(
        <div className={'h-[400px] w-[300px] justify-top rounded-[10px] bg-[#F0F0F0] mr-[70px] ml-[70px] flex flex-col'}>
            <div>
                <img src = {modm} alt={materia.id} className={'w-[300px] h-[250px] rounded-t-lg'}/>
            </div>

            <div className={'text-[15px] justify-left pl-3 pt-2  font-bold flex flex-col'}>
                <div className="text-3xl">{materia.name}</div>
                <div>{materia.group_number}</div>
                <div>Profesor a evaluar: {materia.teacher_name}</div>
            </div>
